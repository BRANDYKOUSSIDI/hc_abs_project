...
export default function BookingForm({ slot, provider, onBooked = () => {} }) {
  ...
  // Auth modal state + pending booking data (used to continue booking after auth)
  const [showAuth, setShowAuth] = useState(false);
  const pendingRef = useRef(null);

  useEffect(() => {
    // if slot or provider changes, clear status
    setStatus(null);
  }, [slot, provider]);
  ...
  const proceedBooking = async (data) => {
    ...
    try {
      ...
      // trigger parent refresh so UI reflects current slot state
      try {
  ...
  const handleSubmit = async (e) => {
    ...
    // If not authenticated, open AuthModal and save pending data
    if (!isAuthenticated()) {
  ...
  const handleAuthSuccess = async () => {
    // called after AuthModal reports success (user logged in / registered)
    setShowAuth(false);
    ...
    <>
      <form className="booking-form" onSubmit={handleSubmit}>
        <div className="slot-summary">
          <div className="slot-date">{new Date(slot.start).toLocaleString()}</div>
